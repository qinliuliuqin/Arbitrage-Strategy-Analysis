...
class LdaSeqModel(utils.SaveLoad):
    ...
    def __init__(
            ...
            random_state=None, lda_inference_max_iter=25, em_min_iter=6, em_max_iter=20, chunksize=100, seeds=None, pi=0.5
        ):
        """
        ...
        print(f"seeds length:{len(seeds)}  pi:{pi}")  
        ...
        # Generate log beta for seed words
        log_beta_s = np.zeros((self.vocab_len, num_topics)) + np.log(1e-8)
        print('Generate seed beta!')
        self.insert_seeds(seeds, log_beta_s)
        print('\n-------Seed beta_s----------')        
        self.print_beta(log_beta_s, 5, self.id2word)
        print('\n')
        ...
        if corpus is not None and time_slice is not None:
            ...
            # fit DTM
            self.fit_lda_seq(corpus, lda_inference_max_iter, em_min_iter, em_max_iter, chunksize, log_beta_s, pi)
    ...
    def fit_lda_seq(self, corpus, lda_inference_max_iter, em_min_iter, em_max_iter, chunksize, log_beta_s, pi):
       ...
       """
        ...
        while iter_ < em_min_iter or ((convergence > LDASQE_EM_THRESHOLD) and iter_ <= em_max_iter):
            ...
            bound, gammas = \
                self.lda_seq_infer(corpus, topic_suffstats, gammas, lhoods, iter_, lda_inference_max_iter, chunksize, log_beta_s, pi)
            self.gammas = gammas
            ...
            print("iteration %i iteration lda seq bound is %f convergence is %f" % (iter_, bound, convergence))
    ...
    def lda_seq_infer(self, corpus, topic_suffstats, gammas, lhoods,
                      iter_, lda_inference_max_iter, chunksize, log_beta_s, pi):
        ...
        ldapost = LdaPost(max_doc_len=self.max_doc_len, num_topics=num_topics, lda=lda, pi=pi)

        model = "DTM"
        if model == "DTM":
            bound, gammas = self.inferDTMseq(
                corpus, topic_suffstats, gammas, lhoods, lda,
                ldapost, iter_, bound, lda_inference_max_iter, chunksize, log_beta_s, pi
            )
        elif model == "DIM":
    ...
    def print_beta(self, beta, n_words, id2word):
        # Assume beta dim(V,T)
        ...
        for k in range(K):
            ...
            a = np.argsort(-t2[k,:])
            for id in a:
                print('{} {:.5f}'.format(id2word[id],np.exp(t2[k,id]))) 
                i += 1
                if i > n_words:
                        break
    
    def insert_seeds(self, seeds, beta_s):
    ...
    def print_report(self, time, n):
        #print(f"shape{self.topic_chains[0].e_log_prob.shape}")
        print("------print_report-------")
        x = np.zeros((self.vocab_len, self.num_topics))
        for k in range(self.num_topics):
            x[:, k] = self.topic_chains[k].e_log_prob[:, time]
        self.print_beta(x, n, self.id2word)
        print("\n")
    ...
    def inferDTMseq(self, corpus, topic_suffstats, gammas, lhoods, lda,
        ...
        #print(f"lda.topics size:{lda.topics.shape}")
       
        #self.insert_seeds(seeds, lda.topics)        
        #print('beta_r')
        #self.print_beta(lda.topics, 5, self.id2word)
        
        #lda.topics = np.log(np.exp(lda.topics) + np.exp(log_beta_s))
        #print('beta_r + beta_s')
        #self.print_beta(lda.topics, 5, self.id2word)
            ...
            for doc in chunk:
                ...
                if iter_ == 0:
                    ...
                    doc_lhood = LdaPost.fit_lda_post(
                        ldapost, doc_num, time, None, lda_inference_max_iter=lda_inference_max_iter, log_beta_s=log_beta_s, pi=pi
                    )
                else:
                    
                    doc_lhood = LdaPost.fit_lda_post(
                      
                        ldapost, doc_num, time, self, lda_inference_max_iter=lda_inference_max_iter, log_beta_s=log_beta_s, pi=pi
                    )
                #t2 = sys_time.time()
                #print(f"LdaPost.fit_lda_post took {t2-t1:.3f} seconds")
...
class LdaPost(utils.SaveLoad):
    ...
    def __init__(self, doc=None, lda=None, max_doc_len=None, num_topics=None, gamma=None, lhood=None, pi=0.5):
        ...
        # doc level eta
        self.eta = np.ones(max_doc_len) * 0.5
        print(f'{self.eta=}')
        pi_k = np.ones(num_topics) * pi
        print('PI:{}'.format(pi_k))
        self.log_pi = np.log(pi_k) 
        self.log_one_minus_pi = np.log(1 - pi_k)
    ...
    def update_phi(self, doc_number, time):
        ...
        **Lee, Seung: Algorithms for non-negative matrix factorization, NIPS 2001**.
        ...
        """
        # print(f'log_beta_s shape{self.log_beta_s.shape} pi:{self.pi}')
        ...
        for word_id, count in self.doc:
            ...
            b = 1 - a
            for k in range(num_topics):
                t1 = b * self.lda.topics[word_id][k] + a * self.log_beta_s[word_id][k] + b * self.log_one_minus_pi[k] + a * self.log_pi[k]
                self.log_phi[n][k] = dig[k] + t1
    ...
    def update_eta(self):
        ...
        n = 0   # keep track of iterations for phi, log_phi
        #print('eta bf:{}'.format(self.eta))
         
        for word_id, count in self.doc:
            t_sum = 0
            for k in range(self.lda.num_topics):
                t1 = self.log_beta_s[word_id][k] + self.log_pi[k] - self.lda.topics[word_id][k] - self.log_one_minus_pi[k]
                t2 = np.exp(self.log_phi[n][k]) * t1
                t_sum += t2
                
            self.eta[n] = np.exp(-np.logaddexp(0, -t_sum))
            n += 1    

        #print('eta af:{}'.format(self.eta))
    ...
    def fit_lda_post(self, doc_number, time, ldaseq, LDA_INFERENCE_CONVERGED=1e-8,
                    lda_inference_max_iter=25, log_beta_s=None, pi=0.5, g=None, g3_matrix=None, g4_matrix=None, g5_matrix=None):
