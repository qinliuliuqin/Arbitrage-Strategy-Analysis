        ...
        **Lee, Seung: Algorithms for non-negative matrix factorization, NIPS 2001**.
        ...
        for word_id, count in self.doc:
            for k in range(num_topics):
                ...
                #lda.topics: regular topic words distribution in log domain 
                #log_beta_s: seed topic words distribution in log domain   
                t1 = (1 - self.pi) * np.exp(self.lda.topics[word_id][k])
                t2 = self.pi * np.exp(self.log_beta_s[word_id][k])
                t3 = np.log(t1 + t2)
                self.log_phi[n][k] = dig[k] + t3
